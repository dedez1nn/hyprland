...
MANUAL_PLACE: Optional[str] = ""  # Set your city here, e.g. "São Paulo, Brazil"
MANUAL_LAT: Optional[float] = None  # Optional: set latitude, e.g. -23.5505
MANUAL_LON: Optional[float] = None  # Optional: set longitude, e.g. -46.6333
...
def get_coords() -> Tuple[float, float]:
    # 1) Hardcoded coordinates (highest priority, no external calls)
    ...
    # 3) Forward geocode from MANUAL_PLACE or ENV_PLACE
    for place in filter(None, [MANUAL_PLACE, ENV_PLACE]):
        coords = get_coords_from_place_name(place.strip())
        if coords:
            return coords
